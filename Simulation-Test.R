...
for(p.choose in 1:3){
  ...
  for(round in 1:500){
    ...
    TrueR=1
    TrueB=1
    while(abs(TrueR)>0.01 | abs(TrueB)>0.1){
    ...
    #################
    error.B[round] = (B.hat-rho.B>TrueB) | (TrueB>B.hat+rho.B)
    error.Q[round] = (Q1.hat-rho.Q1>TrueQ1) | (TrueQ1>Q1.hat+rho.Q1)
    error.R[round] = (R.hat-rho.R>TrueR) | (TrueR>R.hat+rho.R)
    
    power.B[round] = (B.hat-rho.B>0) | (0>B.hat+rho.B)
    power.Q[round] = (Q1.hat-rho.Q1>0)
    power.R[round] = (R.hat-rho.R>0) | (0>R.hat+rho.R)
  }
  
  print(round(c(mean(power.B), mean(power.Q), mean(power.R, na.rm=T), mean(error.B), mean(error.Q), mean(error.R, na.rm=T)),3))
